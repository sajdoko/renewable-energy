	...
	function renewable_energy_bootstrap_comment_form_fields( $fields ) {
		...
		$fields    = array(
			'author' => '<div class="form-group comment-form-author"><label for="author">' . __('Name',
					'renewable_energy') . ( $req ? ' <span class="required">*</span>' : '') . '</label> ' .
...
if ( ! function_exists('renewable_energy_bootstrap_comment_form') ) {
	...
	function renewable_energy_bootstrap_comment_form( $args ) {
		$args['comment_field'] = '<div class="form-group comment-form-comment">
	    <label for="comment">' . _x('Comment', 'noun', 'renewable_energy') . (' <span class="required">*</span>') . '</label>
